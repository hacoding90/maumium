...
export default function ProfileModal({ profile, onClose, onLike, onDelete, isAdmin }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(40,15,25,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 24, maxWidth: 480, width: '100%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>

        <div style={{ background: 'linear-gradient(135deg,#fdf0f4,#f0d8e8)', padding: '32px 24px 20px', borderRadius: '24px 24px 0 0', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
          <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(255,255,255,0.7)', border: 'none', borderRadius: '50%', width: 32, height: 32, cursor: 'pointer', fontSize: 18, color: '#9c6278', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
        ...
        <div style={{ padding: '20px 24px 28px' }}>
          {profile.intro && (
            <div style={{ background: '#fdf6f9', borderRadius: 14, padding: '14px 16px', marginBottom: 18 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#c97090', marginBottom: 6 }}>자기소개</div>
          ...
          <div style={{ background: '#fdf6f9', borderRadius: 14, padding: '14px 16px', marginBottom: isAdmin ? 20 : 0 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#c97090', marginBottom: 10 }}>이상형</div>
            ...
            {profile.idealDesc && <p style={{ margin: 0, fontSize: 14, color: '#4a2535', lineHeight: 1.7, borderTop: '1px solid #f0dce6', paddingTop: 8, marginTop: 4 }}>{profile.idealDesc}</p>}
          </div>
