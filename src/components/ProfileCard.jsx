...
export default function ProfileCard({ profile, onOpen, onLike, isAdmin, onDragStart }) {
  const [hovered, setHovered] = useState(false)

  return (
    <div
      draggable={isAdmin}
      onDragStart={e => onDragStart && onDragStart(e, profile)}
      ...
      style={{
        ...
        border: '1px solid #f5e0e8', cursor: isAdmin ? 'grab' : 'pointer',
    ...
    >
      <div style={{
        ...
        {isAdmin && (
          <div style={{
            ...
            fontSize: 10, color: '#c97090', background: 'rgba(255,255,255,0.8)',
        ...
        <button
          ...
          style={{
            ...
            boxShadow: '0 1px 4px rgba(0,0,0,0.12)', fontSize: 16,
          }}
        >
          {profile.liked ? '❤️' : '🤍'}
        </button>
        <div style={{ marginTop: 10, fontWeight: 700, fontSize: 17, color: '#2d1a22' }}>
          {profile.name}
        </div>
      ...
      <div style={{ padding: '14px 18px 18px', display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
        ...
        <p style={{
          margin: 0, fontSize: 13, color: '#6b4458', lineHeight: 1.6,
        ...
        <div style={{ paddingTop: 10, borderTop: '1px solid #f5e0e8' }}>
          ...
          <div style={{
            fontSize: 12, color: '#7a4a5e', lineHeight: 1.5,
            overflow: 'hidden', display: '-webkit-box',
            WebkitLineClamp: 2, WebkitBoxOrient: 'vertical',
          }}>
