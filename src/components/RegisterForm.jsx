...
export default function RegisterForm({ onSubmit, onCancel, loading }) {
  ...
  const validateStep1 = () => {
    ...
    if (!form.age || form.age < 18 || form.age > 80) errs.age = '나이를 올바르게 입력해주세요 (18-80)'
      ...
      <div style={{ padding: '24px 28px' }}>
        {step === 1 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 14px' }}>
            ...
            <Field label="나이 *">
              <input style={{ ...inputStyle, borderColor: errors.age ? '#e05a7a' : '#f0dce6' }} type="number" value={form.age} onChange={e => set('age', e.target.value)} placeholder="28" min="18" max="80" />
              {errors.age && <div style={{ fontSize: 11, color: '#e05a7a', marginTop: 4 }}>{errors.age}</div>}
            </Field>
            <Field label="키 (cm) *">
              <input style={{ ...inputStyle, borderColor: errors.height ? '#e05a7a' : '#f0dce6' }} type="number" value={form.height} onChange={e => set('height', e.target.value)} placeholder="165" min="140" max="220" />
              {errors.height && <div style={{ fontSize: 11, color: '#e05a7a', marginTop: 4 }}>{errors.height}</div>}
            </Field>
            <Field label="거주 지역">
              <select style={selectStyle} value={form.region} onChange={e => set('region', e.target.value)}>{REGIONS.map(r => <option key={r}>{r}</option>)}</select>
            </Field>
            <Field label="직업">
              <select style={selectStyle} value={form.job} onChange={e => set('job', e.target.value)}>{JOBS.map(j => <option key={j}>{j}</option>)}</select>
            </Field>
            <Field label="학력" full>
              <select style={selectStyle} value={form.education} onChange={e => set('education', e.target.value)}>{EDUCATIONS.map(e => <option key={e}>{e}</option>)}</select>
            </Field>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 14px' }}>
            ...
            <Field label="자기소개" full>
              <textarea style={{ ...inputStyle, height: 120, resize: 'vertical', lineHeight: 1.65 }} value={form.intro} onChange={e => set('intro', e.target.value)} placeholder="나를 한마디로 표현하자면..." maxLength={500} />
        ...
        {step === 3 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 14px' }}>
            ...
            <Field label="직업 관련 선호" full><input style={inputStyle} value={form.idealJob} onChange={e => set('idealJob', e.target.value)} placeholder="예: 안정적인 직업이면 좋겠어요" maxLength={100} /></Field>
            <Field label="이상형 한 줄 소개" full>
              <textarea style={{ ...inputStyle, height: 100, resize: 'vertical', lineHeight: 1.65 }} value={form.idealDesc} onChange={e => set('idealDesc', e.target.value)} placeholder="예: 유머 감각 있고 가족을 소중히 여기는 분" maxLength={300} />
